...
for tests. It does not start a shell, helper process, file picker, daemon, or
clipboard watcher.
...
- The primary composer flow is `Ctrl-v` for paste/import, `Delete` for the
  newest attachment, `Ctrl-r` to retry failed uploads, `Ctrl-c` to confirmably
  clear, and `Ctrl-o` for the clearly labelled local-path fallback. `Ctrl-a`
  no longer claims an attachment role.
...
- Native clipboard access may be unavailable in a remote, sandboxed, or
  ...
  to a shell command; `Ctrl-o` and terminal text paste remain available.
- The dependency is pinned to `arboard 3.6.1` (MIT OR Apache-2.0, Rust 1.71
